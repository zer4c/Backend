...
import src.modules.users.service as us
...
def get_all_users_response(session, city, country, email):
    ...
    response = IResponse(detail="users found", status_code=200, data=users_dto)
    return response
...
def get_user_response(id: int, session):
    if id is None:
        raise exceptions.HTTPException(
            status_code=400, detail="User ID must not be None."
        )
    ...
    response = IResponse(
        detail="user founded succesfully", status_code=200, data=user_dto
    )
...
def update_user_response(user: User, id: int, session):
    ...
    user_dto = __convert_DTO(user)
    response = IResponse(
        detail="user updated succesfully", status_code=200, data=user_dto
    )
    return response
...
def patch_user_response(user: User, id: int, session):
    ...
    if not any(
        ...
        raise exceptions.HTTPException(status_code=400, detail="No fields to update.")
    ...
    response = IResponse(
        detail="User patched successfully", status_code=200, data=user_dto
    )
    return response
